...
### Changed
- `jsw-vanna` 改为在上下文版本变化时一次性预计算并持久化表、字段和历史 SQL chunk 的 embedding，后续生成 SQL 时只对用户问句计算 1 次向量。
- Vanna 检索链路优先复用 PostgreSQL 中已持久化的 chunk 向量做相似度排序；当持久化向量缺失、损坏或模型维度不匹配时，会自动回填并在必要时回退到原有内存编码逻辑。
- Vanna 启动初始化会自动为现有库补齐 `embedding_model_key`、`embedding_values`、`embedding_dims` 字段，兼容旧环境升级，同时保留旧版 `vector(1536)` 列不再参与新逻辑。

### Tests
- 新增 Vanna 向量缓存与检索单测，覆盖上下文未变跳过重算、模型标识变化触发重建、持久化向量命中、懒回填和坏数据跳过场景。
- 新增 Vanna 数据库初始化升级测试，覆盖现有表结构在启动时自动补列行为。

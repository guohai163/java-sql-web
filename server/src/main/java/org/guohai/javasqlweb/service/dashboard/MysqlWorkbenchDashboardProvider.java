...
public class MysqlWorkbenchDashboardProvider extends AbstractWorkbenchDashboardProvider {

    @Override
    public String getDbType() {
        return DbServerTypeUtils.MYSQL;
    }
    ...
    public List<WorkbenchDashboardSection> buildSections(DbOperation operation, String dbName, ConnectConfigBean config) {
        return List.of(
                section("system", "系统信息",
                        ...
                        queryItem(operation, dbName, "threads_running", "运行线程数", "SHOW GLOBAL STATUS LIKE 'Threads_running'", "Value")
                ),
                section("database", "数据库信息",
                        ...
                        queryItem(operation, dbName, "questions", "Questions 累计", "SHOW GLOBAL STATUS LIKE 'Questions'", "Value")
                ),
                section("extra", "其他信息",
                        ...
                        queryItem(operation, dbName, "buffer_pool_size", "InnoDB Buffer Pool(bytes)", "SHOW GLOBAL VARIABLES LIKE 'innodb_buffer_pool_size'", "Value")
                )
        );
    }
}
